...
Then $E(M) = E(D_{1}) + E(D_{2}) +... +E(D_{n}) = (1)/(\lambda) sum_(i=1)^(n) (1)/(i) $ 
...
$\mathrm{Var}(M) = \mathrm{Var}(D_1 + D_2 + ... + D_n)$

$\mathrm{Var}(M) = \mathrm{Var}(D_1) + \mathrm{Var}(D_2) + ... + \mathrm{Var}(D_n)$

Since $\mathrm{Var}(D_i) = 1 / [ (n-i+1)^2 \lambda^2 ]$,

$
    \mathrm{Var}(M) = \sum_{i=1}^n (1)/((n-i+1)^2 \lambda^2) = (1)/(\lambda^2) \sum_{i=1}^n (1)/(i^2)
$