...
+ Let $X$ be the number of people that play the same opponent in both
rounds. Let $I_{j}$ be the indicator variable that person $j$ plays against the
same opponent twice. $P(I_{j}=1) = (1)/(99)$. Then, $\text{E}(X) = \sum_
{j=1}^{100}P(I_{j}=1) = 100/99$.

+ There is a strong dependence between trials. For instance, if we know that
...
+ Consider the $50$ pairs that played each other in round one. Let $I_{j}$
be the indicator variable for pair $j$ playing each other again in the second
round. $P(I_{j}=1) = (1)/(99)$. Then, the expected number of pairs that
play the same opponent twice is $\text{E}(Z) = (50)/(99) approx (1)/(2)$.

We can approximate the number of pairs that play against one another in both
rounds with $Z \sim \text{Poiss}((1)/(2))$. Note that $X=2Z$. $P(X = 0) approx P(Z =
0) = e^{-(1)/(2)} approx 0.6$.

$P(X = 2) approx P(Z = 1) = ((\frac{1)/(2))^{1}e^{-(1)/(2)}}{1!}
approx 0.32$.
