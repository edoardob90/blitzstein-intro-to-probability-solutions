...
definition of medians, $P(X lt.eq m) gt.eq (1)/(2)$ and $P(X gt.eq m_{1}) gt.eq 
(1)/(2)$. Then, $P(X in (m,m_{1})) = 0$. If $m_{1} > m + 1$, then
...
+ Let $I_{j}$ be the indicator variable for the event $X gt.eq j$. Notice
that the event $X = k$ (the first occurance of a birthday match happens when
there are $k$ people) implies that $I_{j}=1$ for $j lt.eq k$ and vice versa.
...
+ $\text{E}(X^{2}) =
\text{E}(I_{1}^{2} + ... + I_{366}^{2} + 2\sum_{j=2}^
{366}sum_(i=1)^(j-1)(I_{i}I_{j}))$. Note that $I_{i}^{2} = I_{i}$ and $I_{i}I_
{j} = I_{j}$ for $i < j$. Thus,

$
\text{E}(X^{2}) = \text{E}(I_{1} + ... + I_{366} + 2\sum_{j=2}^
{366}sum_(i=1)^(j-1)I_{j}) 

= 2 + sum_(j=3)^(366)p_{j} + 2sum_(j=2)^(366)((j-1)\text{E}(I_{j})) 

= 2 + sum_(j=3)^(366)p_{j} + 2sum_(j=2)^(366)((j-1)p_{j}) 
