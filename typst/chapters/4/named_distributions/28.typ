...
By symmetry. $P(I_{k}=1) = (t)/(n)$. Then,

$P(X = k) = P(I_{k}=1)P(X_{k-1}=t-1) = (t)/(n)\frac{\binom{t-1}
{t-1}binom(n-1-t+1, k-1-t+1)}{
binom(n-1, k-1)} = (t)/(n)(binom(n-t, k-t))/(binom(n-1, k-1))$

$\text{E}(X) = sum_(k=t)^(n)k P(I_{k}=1)P(X_{k-1}=t-1) = sum_(k=t)^(n)(k
(t)/(n)(binom(n-t, k-t))/(binom(n-1, k-1))) = ((n+1)t)/(t+1)$