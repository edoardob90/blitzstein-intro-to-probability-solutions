...
+ Consider the simple case of $m < (n)/(2)$. Then, the trays don't have enough
pages to print $n$ copies. Desired probability is $0$.

On the other hand, if $m gt.eq n$, then desired probability is $1$, since each
tray individually has enough pages.

Now, consider the more interesting case that $(n)/(2) lt.eq m < n$.
...
$ P = \begin{cases} 
      ...
      pbinom(m,n,p) - pbinom(n-m-1,n,p)  (n)/(2) lt.eq m < n 

      1  m gt.eq n
   \end{cases}
$

+ Typing out the hinted program in the R language, we get that the smallest
