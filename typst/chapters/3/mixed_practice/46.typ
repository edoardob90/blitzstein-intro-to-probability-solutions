...
+ If we think of the Bernoulli trial success as a win for player $A$, and
...
+ $p_{k} = (1)/(2)p_{k+2} + (1)/(2)p_{k-1}$ with conditions $p_{0} =
1$ and
$\lim_{k arrow.r infinity} p_{k} = 0$

The characteristic equation is $(1)/(2)t^{3} - t + (1)/(2) = 0$ with
...
Using the hint that $\lim_{k arrow.r infinity} p_{k} = 0$, $c_{1}$ and $c_{3}$
must be $0$. Thus, 
