...
+ Let $Y$ be the number of Statistics students in the sample of size $m$.

$P(Y=k) = sum_(i=k)^(n)P(X=i)P(Y=k|X=i) = sum_(i=k)^(n)binom(n, i)p^{i}
(1-p)^{n-i}(binom(i, k)binom(n-i, m-k))/(binom(n, m))$

+ Consider a student in a random sample of size $m$. Independently of
