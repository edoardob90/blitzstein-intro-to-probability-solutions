...
+ The distribution is hypergeometric. We select a sample of $t$ employees
and count the number of women in the sample.

$P(X = k) = (binom(n, k)binom(m, t-k))/(binom(n+m, t))$

+ Decisions to be promoted or not are independent from employee to employee.
...
+ Once the total number of promotions is fixed, they are no longer
...
{t-k}(1-p)^{m-t+k}}{binom(n+m, t)p^{t}(1-p)^{n+m-t}} = (binom(n, k)
