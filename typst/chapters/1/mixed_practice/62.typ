...
+ $1 - k!e_{k}\left(\overrightarrow{p}\right)$

+ Consider the extreme case where $p_{1} = 1$ and $p_{i} = 0$ for $i eq.not 1$. 
Then, the probability that there is at least one birthday match is $1$. 
In general, if $p_{i} > (1)/(365)$ for a particular $i$, then a 
...
at least one birthday match is minimized when $p_{i} = (1)/(365)$.

+ First, consider $e_{k}\left(x_{1},...,x_{n}\right)$. We can break up this 
sum into the sum of three disjoint cases.
  + 
    
+ Sum of terms that contain both $x_{1}$ and $x_{2}$. This sum is given 
    by $x_{1}x_{2}e_{k-2}\left(x_{3},...,x_{n}\right)$

    
+ Sum of terms that contain either $x_{1}$ or $x_{2}$ but not both. 
    This sum is given by $\left(x_{1} + x_{2}\right)e_{k-1}\left(x_{3},...,x_{n}\right)$

    
+ Sum of terms that don't contain either $x_{1}$ or $x_{2}$. This sum 
  ...
  $r_{1}r_{2}e_{k-2}(x_{3},...,x_{n}) gt.eq p_{1}p_{2}e_{k-2}(x_{3},...,x_{n})$. 
  Hence, $1 - k!e_{k}(\overrightarrow{p}) gt.eq 1 - k!e_{k}(\overrightarrow{r})$.
