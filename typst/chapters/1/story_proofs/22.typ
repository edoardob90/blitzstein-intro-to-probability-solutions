...
+ Let us count the number of games in a round-robin tournament with $n+1$ 
...
+ LHS: If $n$ is chosen first, then the subsequent $3$ numbers can be any 
of $0, 1, ..., n-1$. These $3$ numbers are chosen with replacement resulting 
in $n^{3}$ possibilities. Summing over possible values of 
$n$ we get $1^{3} + 2^{3} + ... + n^{3}$ total number of possibilities.
