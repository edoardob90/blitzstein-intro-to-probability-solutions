...
+ Both Fred and his friend are correct. Fred is correct in that the
...
+ $P(F|H_{1 lt.eq i lt.eq 92}) = \frac{P(F)P(H_{1 lt.eq i lt.eq 92}|F)}{P(F)P(H_{1
lt.eq i lt.eq 92}|F) + P(F^{c})P(H_{1lt.eq i lt.eq 92}|F^{c})} = (p\left
(\frac{1)/(2)\right)^{92}}{p\left((1)/(2)\right)^{92} + (1-p)}$

+ For $P(F|H_{1 lt.eq i lt.eq 92})$ to be larger than $(1)/(2)$, $p$ must be
greater than $\frac{2^{92}}{2^{92} + 1}$, which is approximately equal to $1$,
where as for $P(F|H_{1 lt.eq i lt.eq 92})$ to be less than $(1)/(20)$, $p$
